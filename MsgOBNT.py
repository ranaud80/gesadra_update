...
class FormOBNT:
    ...
    def txtFileOBNT(self):
        ...
        # Corps du message
        fic.write("Objet        : " + Commun.encode(self.efObjet.get())+"\n")
        fic.write("Référence    : " + Commun.encode(self.eRef.get())+"\n")
        fic.write("Message : \n\n")
        # découpage des lignes du message
        for ligne in Commun.encode(self.stMessage.get(1.0,END)).split("\n"):
            if len(ligne) > 79:
    ...
    def xmlFileOBNT(self):
        ...
        fic.write("<ref>" + Commun.encode(self.eRef.get())+"</ref>\n")
        # découpage des lignes du message
        for ligne in Commun.encode(self.stMessage.get(1.0,END)).split("\n"):
            if len(ligne) > 79:
                for ligne in textwrap.wrap(ligne,79):
